...
# ── 문서 업로드 : 파일 청크 및 업로드 ─────────────────────────────────────────
    ...
    if uploaded_file is not None:
        ...
        chunk_size = st.slider("청크 크기 (문자 수)", min_value=200, max_value=7500, value=800, step=100)
        overlap = st.slider("오버랩 (중복 문자 수)", min_value=0, max_value=500, value=100, step=50)

        if st.button("업로드",use_container_width=True):

            try:
                chunks = chunk_text(file_contents, chunk_size=chunk_size, overlap=overlap)
                blob_service_client = BlobServiceClient.from_connection_string(connection_string)
                uploaded_files = upload_chunk_to_blob(blob_service_client, container_name, file_name, chunks)
                
                st.success(f"✅ `{uploaded_file.name}` 파일이 Blob Container에 업로드되었습니다.")

            except Exception as e:
                st.error(f"❌ 업로드 실패: {str(e)}")
        
# ── 문서 분석 : 인덱스 > 인덱서 > RAG 검색  ─────────────────────────────────────────
...
# ── 문제 생성  ─────────────────────────────────────────
    ...
    # 문제 난이도 선택 드롭다운 추가
    problem_level = st.selectbox("문제 난이도를 선택하세요:", ("랜덤", "초급", "중급", "고급"))

    # 문제 개수 입력 받기
    problem_count = st.number_input("생성할 문제 개수를 입력하세요:", min_value=0, max_value=100, value=2)

    detail = ""
    if problem_type == "정보처리기사":
        detail = st.text_input("(선택) 구체적으로 물어보세요:")
...
# ── 생성된 문제 풀기  ─────────────────────────────────────────
    ...
    if "user_answers" not in st.session_state or len(st.session_state.user_answers) != len(df_clean):
    ...
    with col3:
        if st.button("다음 ➡️", disabled=(idx == len(df_clean) - 1), use_container_width=True):
